...
public interface UserRepository extends Neo4jRepository<UserNode, String> {
    ...
    @Query("Match (u:UserNode),(q:QuestionNode),(a:AnswerNode),(c:children),(p:parents) where u.username={username} and (u)-[:follows]->(c) or (u)-[:follows]->(p) and (q)-[:question_of_topic]->(c) or (q)-[:question_of_topic]->(p)  and (a)-[:answer_of]->(q) and a.accepted=true return q")
