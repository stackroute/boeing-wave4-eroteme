...
public interface UserRepository extends Neo4jRepository<User, String> {
    ...
    //method to get USER//
    @Query("MATCH (m:UserDTO) RETURN m")
    Collection<User> getAllUsers();


    //method to get USER using REPUTATION//
    @Query("match (m:UserDTO) where m.reputation={reputation} return m")
    User getByUser(@Param("reputation") int reputation);


    //method to create relationship FOLLOWS between userDTO and topic//
    @Query("match (q:UserDTO),(t:parents) where q.userName={username} and t.name={name} create (q)-[r:follows]->(t)")
